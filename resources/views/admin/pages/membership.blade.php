    ...
    <div class="pc-content">
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Membership Requests</h5>
                        </div>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="javascript: void(0)">Management</a></li>
                            <li class="breadcrumb-item" aria-current="page">Membership Requests</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5>All Membership Requests</h5>
                <span class="badge bg-warning text-dark" id="pendingCount" style="display:none;"></span>
                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addMembershipPlanModal">
    + Add Membership Plan
</button>
            </div>
...
<td>
    ...
    @empty
        <tr>
            <td colspan="9" class="text-center text-muted py-4">No membership requests yet.</td>
        </tr>
    @endforelse
</tbody>
                </table>
...
<div class="modal fade" id="addMembershipPlanModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Add Membership Plan</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div id="planFormError" class="alert alert-danger d-none"></div>
                <div class="mb-3">
                    <label class="form-label">Plan Name</label>
                    <input type="text" class="form-control" id="planName" placeholder="e.g. Gold">
                </div>
                <div class="row g-2">
                    <div class="col-6">
                        <label class="form-label">Price ($)</label>
                        <input type="number" class="form-control" id="planPrice" placeholder="0.00" min="0" step="0.01">
                    </div>
                    <div class="col-6">
                        <label class="form-label">Duration (days)</label>
                        <input type="number" class="form-control" id="planDuration" placeholder="30" min="1">
                    </div>
                </div>
                <div class="mt-3">
                    <label class="form-label">Features <small class="text-muted">(one per line)</small></label>
                    <textarea class="form-control" id="planFeatures" rows="5"
...
(function () {
    ...
    window.membershipAction = function (id, action) {
        ...
        document.getElementById('adminNotesModalTitle').textContent =
            action === 'approve' ? 'Approve Membership' : 'Reject Membership (reason required)';
        const confirmBtn = document.getElementById('adminNotesConfirm');
        confirmBtn.textContent = action === 'approve' ? 'Approve' : 'Reject';
        confirmBtn.className   = action === 'approve' ? 'btn btn-success btn-sm' : 'btn btn-danger btn-sm';
        document.getElementById('adminNotesInput').value = '';
        document.getElementById('adminNotesInput').classList.remove('is-invalid');
        new bootstrap.Modal(document.getElementById('adminNotesModal')).show();
    };

    document.getElementById('adminNotesConfirm').addEventListener('click', async function () {
        if (!pendingAction) return;
        const { id, action } = pendingAction;
        const notes = document.getElementById('adminNotesInput').value.trim();

        if (action === 'reject' && !notes) {
            document.getElementById('adminNotesInput').classList.add('is-invalid');
            return;
        }
        ...
        try {
            const res  = await fetch(`/admin/membership-requests/${id}/${action}`, {
                method: 'POST',
    ...
    document.getElementById('adminNotesInput').addEventListener('input', function () {
...
document.getElementById('savePlanBtn').addEventListener('click', async function () {
    ...
    if (!name || !price || !duration) {
        $err.textContent = 'Name, price and duration are required.';
    ...
    try {
        ...
        if (!res.ok || !data.status) {
            $err.textContent = data.message || 'Failed to save plan.';
            $err.classList.remove('d-none');
            return;
        }

        bootstrap.Modal.getInstance(document.getElementById('addMembershipPlanModal')).hide();
        location.reload();
