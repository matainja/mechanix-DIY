...
{
    /**
     * Request cooldown period in hours (24 hours = 1 day)
     * Change this value as needed: 24 = 1 day, 48 = 2 days, 720 = 1 month
     */
    private const REQUEST_COOLDOWN_HOURS = 24;
    ...
    public function submitRequest(Request $request)
    {
        ...
        try {
            // Check for existing pending request within cooldown period
            $existingRequest = MembershipRequest::where('user_id', auth()->id())
                ->where('status', 'pending')
                ->where('created_at', '>=', now()->subHours(self::REQUEST_COOLDOWN_HOURS))
                ->latest()
                ->first();

            if ($existingRequest) {
                $hoursLeft = now()->diffInHours($existingRequest->created_at->addHours(self::REQUEST_COOLDOWN_HOURS));
                
                return response()->json([
                    'status' => false,
                    'message' => "Your request for {$existingRequest->membershipPlan->name} is already pending. Please wait {$hoursLeft} more hours before submitting a new request.",
                    'pending_request' => [
                        'plan_name' => $existingRequest->membershipPlan->name,
                        'created_at' => $existingRequest->created_at->format('M d, Y h:i A'),
            ...
            // Delete any old pending requests for this user (older than cooldown period)
            MembershipRequest::where('user_id', auth()->id())
                ->where('status', 'pending')
                ->where('created_at', '<', now()->subHours(self::REQUEST_COOLDOWN_HOURS))
                ->delete();
    ...
    public function submitGuestRequest(Request $request)
    {
        ...
        try {
            // Check for existing pending request within cooldown period (by email)
            $existingRequest = MembershipRequest::where('guest_email', $validated['guest_email'])
                ->where('status', 'pending')
                ->where('created_at', '>=', now()->subHours(self::REQUEST_COOLDOWN_HOURS))
                ->latest()
                ->first();
            ...
            // Delete any old pending requests for this email (older than cooldown period)
            MembershipRequest::where('guest_email', $validated['guest_email'])
                ->where('status', 'pending')
                ->where('created_at', '<', now()->subHours(self::REQUEST_COOLDOWN_HOURS))
                ->delete();
    ...
    public function approveRequest(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($id, $validated) {
            ...
            // Delete previous active membership for this user (if exists)
            if ($membershipRequest->user_id) {
                UserMembership::where('user_id', $membershipRequest->user_id)
                    ->where('status', 'active')
                    ->delete();
            } elseif ($membershipRequest->guest_email) {
                UserMembership::where('guest_email', $membershipRequest->guest_email)
                    ->where('status', 'active')
                    ->delete();
            }
    ...
    public function guestPayment(Request $request)
    {
        ...
        return DB::transaction(function () use ($validated) {
            ...
            // Delete previous active membership for this guest
            if ($membershipRequest->guest_email) {
                UserMembership::where('guest_email', $membershipRequest->guest_email)
                    ->where('status', 'active')
                    ->delete();
            }
